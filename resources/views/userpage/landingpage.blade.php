@php
    $take_survey_exam = DB::table('total_test')
        ->join('clone_strand_category', 'total_test.category_id', '=', 'clone_strand_category.clone_strand_id')
        ->select('total_test.category_id', DB::raw('MAX(total) as max_score'), 'clone_strand_category.category_name')
        ->where('total_test.user_id', Session::get('userid'))
        ->groupBy('total_test.category_id')
        ->get();
    
    $highest_score = 0;
    $highest_category = '';
    
    foreach ($take_survey_exam as $exam) {
        if ($exam->max_score > $highest_score) {
            $highest_score = $exam->max_score;
            $highest_category = $exam->category_name;
        }
    }
    
    $max_score_string = (string) $highest_score;
    $escaped_max_score = htmlspecialchars($max_score_string);
@endphp
                ...
                <ul class="navbar-nav ms-auto ">
                    ...
                    <li class="nav-item">
                        <a class="nav-link mx-2 text-light" aria-current="/announcement_area"
                            href="/announcement_area">Announcements</a>

                    </li>
                    <li class="nav-item">
                        @if (session()->get('take_survey_exam') == 1)
                            <a class="nav-link mx-2 text-light" aria-current="/strand_survey"
                                onclick="take_survey_exam()">Strand
                                Survey
                                List</a>
                        @else
                            <a class="nav-link mx-2 text-light" aria-current="/strand_survey"
                                href="/strand_survey">Strand
                                Survey
                                List</a>
                        @endif
                    </li>
                        ...
                        @if ($taken_exam == 0)
                            <form action="" method="get">
                                <button type="submit" name="takeexam" class="btn ">
                                    <img src="assets/images/exam.png" class="img-button img-fluid edging"
                                        alt="">
                    ...
                    <div class="card-header text-center">
                        @if ($taken_survey == 0)
                            <button type="button" class="btn " data-bs-toggle="modal"
                                ...
                                <img src="assets/images/survey.jfif" class="img-button img-fluid edging"
                                    alt="">
                            ...
                            <div class="row rounded surveyrow m-2 p-2" id="first">
                                @foreach ($take_survey as $number => $item)
                                    <div class="form-group mt-2  rounded survey">
                                        <label for="" class="text-dark"><span
                                                class="fw-bold">{{ $number += 1 }}.
                                            </span>{{ $item->survey_question }}</label>
                                        <input type="text" class="form-control" placeholder="Answer"
                                            name="{{ $item->id }}" value="{{ old('1') }}">
                                        @error('{{ $item->id }}')
                    ...
                    <div class="modal-body">
                        <section>
                            ...
                            <div class="container border"
                                style="height:50vh;overflow-x: hidden;overflow-y: auto; background-color:rgba(255, 255, 255, 0.96);border-radius:50px;">
                                <div class="wrapper">
                                    <div class="form">
                                                ...
                                                <div class="row">
                                                    <div class="col-1">
                                                        <img class="mt-2 img-fluid"
                                                            src="https://th.bing.com/th/id/OIP.WOZKI0uQD-lA_7mQCs3zZQHaHa?pid=ImgDet&rs=1"alt="">
                                                    </div>
                                                    <div class="col-6 mt-2 fs-5 p-2 w-auto bg-info "
                            ...
                            @if ($taken_forum == 1)
                                <div class="container">
                                    <div class="input-group mb-3 mt-2">
                                        <input type="text" id="data" name="mycomment" class="form-control"
                                            placeholder="Enter comment here.." aria-label="Recipient's username"
                                            aria-describedby="button-addon2">
                                        <button id="send-btn" class="btn btn-info" type="submit" disabled
                                ...
                                <div class="container">
                                    <div class="input-group mb-3 mt-2">
                                        <input type="text" id="data" name="mycomment" class="form-control"
                                            placeholder="Enter comment here.." aria-label="Recipient's username"
                                            aria-describedby="button-addon2">
                                        <button id="send-btn" class="btn btn-info" type="submit"
                                            id="button-addon2">Submit</button>
                                    </div>
                                </div>
                            @endif
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body" style="background-image: url('assets/images/mainbg.png');">
                    ...
                    <div class="col-text-center">
                        <p class="h3 text-center text-warning">
                            RECOMMENDED STRAND: <span class="bg-info text-dark"
                                style="padding:0 20px;border-radius:25px;">{{ $recomended }} </span>
                        </p>
                    </div>
                    <div class="row mt-2">
                        <div class="col-md-5 ">
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group rounded text-center"
                                        ...
                                        <label for="" class="text-center mt-1 text-light h5">First
                                            Name</label>
                                        <input type="text" name="Fullname"
                                            class="form-control text-center fw-bold" value="{{ $fullname }}">
                                        @error('Fullname')
                                            <p class="text-danger">
                                                {{ $message }}
                                            </p>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group rounded text-center"
                                        style="background-color:rgba(114, 114, 114, 1);">
                                        <label for="" class="text-center mt-1 text-light h5">Middle
                                            Name</label>
                ...
                <div class="modal-footer bg-secondary">
                    Exam Survey Recommended Strand:
                    {{ $escaped_max_score }} {{ $highest_category }}
