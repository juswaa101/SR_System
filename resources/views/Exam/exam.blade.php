        ...
        <div class="container">
            ...
            <div class=" collapse navbar-collapse" id="navbarNavDropdown">
                <ul class="navbar-nav ms-auto ">
                    ...
                    <li class="nav-item">
                        <form action="" method="get">
                            <button class=" btn nav-link mx-2 text-light" name="stillexam"
                                type="submit">COOKERY</button>
                        </form>
                    </li>
                    <li class="nav-item">
                    ...
                    <li class="nav-item">
                        <form action="" method="get">
                            <button class="btn nav-link mx-2 text-light" name="stillexam" type="submit"><i
                                    class="fa fa-sign-out" style="font-size:30px;color:red"></i></button>
                        </form>
                    </li>
                </ul>
            </div>
    ...
    <div class="container-fluid">
        <div class="row justify-content-center mt-5">
            <div class="col-md-10 examform">
                <form action="pass_exam" class="p-5" method="post">
                    ...
                    @foreach ($exam as $index => $item)
                        <div class="form-group">
                            <label for="{{ $index }}" class="h3 text-dark"><span
                                    class="fw-bold">{{ $index += 1 }}.</span> {{ $item->question }}</label>
                            <div class="container-fluid fs-5">
                                <div class="form-check">
                                    <input class="form-check-input" required type="radio" name="{{ $item->id }}"
                                        id="{{ $item->id }}" value="{{ $item->choice_a }}">
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="{{ $item->id }}"
                                        id="{{ $item->id }}" value="{{ $item->choice_b }}">
                                    <label class="text-dark form-check-label" for="{{ $item->id }}">
                                        {{ $item->choice_b }}
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="{{ $item->id }}"
                                        id="{{ $item->id }}" value="{{ $item->choice_c }}">
                                    <label class="text-dark form-check-label" for="{{ $item->id }}">
                                        {{ $item->choice_c }}
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="{{ $item->id }}"
                                        id="{{ $item->id }}" value="{{ $item->choice_d }}">
                                    <label class="text-dark form-check-label" for="{{ $item->id }}">
                                        {{ $item->choice_d }}
                                    </label>
                                </div>
                ...
                <a class="btn btn-primary" href="#neubar" style="position:fixed;bottom:10px;right:180px;"
                    role="button">Back to Top <i class="fa fa-level-up" style="font-size:20px"></i></a>
                <a class="btn btn-success" href="#bottom" style="position:fixed;bottom:10px;right:320px;"
                    role="button">Bottom <i class="fa fa-caret-square-o-down" style="font-size:20px"></i></a>
            </div>
        </div>
    </div>
    </div>
